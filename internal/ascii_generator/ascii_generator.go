...
func GenerateAsciiArtFromAverages(chunked_average [][][]int, use_color bool) {

	for y_i := 0; y_i < len(chunked_average); y_i++ {
		for x_i := 0; x_i < len(chunked_average[y_i]); x_i++ {
			print_rgb_to_character(chunked_average[y_i][x_i], use_color)
		}
		fmt.Println("")
	}
}

func print_rgb_to_character(rgb_value []int, use_color bool) {
	if len(rgb_value) != 3 {
	...
	if use_color {
		...
		fmt.Print(string(brightness_characters[int(darkness_index)]))
	}
